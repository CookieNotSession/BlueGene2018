...
# Model
trainface = []
os.chdir('Aface Path') # A face dataset path
filelist = os.listdir()
for i in filelist:
...
trainnonface = []
os.chdir('nonA Path') # Non A face dataset path
filelist = os.listdir()
for i in filelist:
    picture = io.imread(i,as_grey=True)
    picture = np.ndarray.flatten(picture)
...
X = np.append(trainface,trainnonface,axis=0) # using array to combine the trainface and trainnonface
y = np.append(np.ones((trainface.shape[0],1)),np.zeros((trainnonface.shape[0],1)),axis=0) # 1D array trainface
...
def get_gallery(dirnn):
    image_names = os.listdir('finalproject\\upload_file_python\\images\\'+dirnn)
    print(image_names)
